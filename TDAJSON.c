...
int jsonGuardar(TDAJSON *TDAJson, char *rutaJson)
{
	...
	/* Acumula la longitud de los atributos cuando deben escribirse llaves, por diseño
	 * Cada posición corresponde a un nivel, en total son 3 niveles
	 * Si la cantidad de niveles fuera variable, en lugar de un vector sería conveniente utilizar el TDA_Pila
	 */
    char Atributo_Llave[CANTMAX];
    char Atributo_Llave_Nivel2[CANTMAX];
    /* Acumula los atributos cuando deben escribirse corchetes
     * Si la cantidad de niveles fuera variable, en lugar de variables sería conveniente utilizar el TDA_Pila
     */
	int CantCierres = 0;
	Atributo_Llave[0] = '\0';
    ...
    /* Cierro el archivo */
	fclose(arch);
	/* Destruyo la estructura TDAJSON */
